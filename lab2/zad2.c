...
void sort(int n, int arr[n], sortType sort_type, bool increase, FILE *fp);
  ...
  // fprintf(fp, "This is testing for fprintf...\n");
   //fclose(fp);	
	...
	if(manual)
	{
		...
		sort(arr_size, nums, sort_type, increase, fp);
	}
	else
	{
		...
		//{
			...
			{
				//fprintf(fp, "n - %5d, i - %5d\n", n, i);
				int* my_table = rand_table(100);
				printf("sorting...\n");
				sort(100, my_table, sort_type, increase, fp);

				my_table = rand_table(1000);				
				printf("sorting...\n");
				sort(1000, my_table, sort_type, increase, fp);

				my_table = rand_table(10000);
				printf("sorting...\n");				
				sort(10000, my_table, sort_type, increase, fp);
...
int* rand_table(int count) {
	...
	{
        ret[i] = (rand()*(int)(&ret[i]))%32767;	//liczba pseudolosowa z uzyciem adresu i rand()
	}
    return ret;
}
...
void sort(int n, int arr[n], sortType sort_type, bool increase, FILE* fp)
{
	...
	printf("Liczba przemieszczen: %d\n\n", swap);
	
	fprintf(fp, "Rozmiar tablicy: %d\n", n);
	fprintf(fp, "Liczba porownan: %d\n", compare);
	fprintf(fp, "Liczba przemieszczen: %d\n", swap);
	fprintf(fp, "Czas dzialani: %g msec\n", msec);
}
